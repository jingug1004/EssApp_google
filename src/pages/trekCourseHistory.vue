  ...
  methods: {
		...
		getTrekCourseHistory() {
			...
			Ajax.run({url:"/leisure/trek/course/log", method:"POST", data:{uid:Manage.cookie.getUid(), tc_id}, token:true}, function(data){
				$$("#popup-trek-course #menuTitle").text(courseName + Manage.lang.trans("DEFINE_TREK_RECORD"));
				$$("#popup-trek-course #pointTxt").text(courseName + " "+Manage.lang.trans("DEFINE_TREK_POINT_RECORD")+" " +(data.total || 0)+ "p");
				let tableHtml = '';
				if(data.list) {
					...
					tableHtml += '<tr>';
					tableHtml += '<td style="text-align: center;">'+Manage.lang.trans("DEFINE_TREK_NO_HISTORY")+'</td>';
					tableHtml += '</tr>';
				}
